...
//add events
			...
			if(location.protocol!="file:" && !$(this).hasClass(customObject.noLoadAjax)){
				...
				if(linkHref.indexOf("#")==-1){
					...
					if($(this).parents("form").length==0){
						...
						hrefDomen = hrefDomenSlice.slice(0, hrefDomenSlice.indexOf("/")).slice(0, hrefDomenSlice.indexOf(":"));
						
						if(webSiteHref.indexOf(hrefDomen)!=-1){
		...
		function loadContent(url, push){
				...
				xhr = $.ajax({
					...
					beforeSend: function () {
					    // init stuff 
					    $("#submitModal").modal('hide'); // close dialog if any
					    $("#myChat").html('');   // clear chat
					...
					success: function(data, textStatus, xhr) {
					    ...
					    // turn off scrolling button
						if (url.toLowerCase().indexOf('matches') < 0) {
						    $('#back-to-bottom').stop(true).animate({ "opacity": 0 }, 400, function () { $(this).css({ "display": "none" }) });
						} else {
					...
					error: function (err) {
					    var requestUrl = history.location || document.location;
					    loadContent(requestUrl.origin + '/' + customObject.errorPage, true);
			  		}
				});
		}
