using System;
using System.Collections.Generic;
using System.Linq;
using System.Web;

namespace quiniela.Models
{
    public class RulesModel
    {
        public string RulesTitle { get; set; }
        public string DomainPath { get; set; }
        public string MyScores { get; set; }
        public string RegTitle { get; set; }
        public string MyScoresTitle{ get; set; }
        public string CalcTitle { get; set; }
        public string WinnerTitle { get; set; }
        public string DonationTitle { get; set; }
        public string Rules1 { get; set; }
        public string Rules2 { get; set; }
        public string Rules3 { get; set; }
        public string Rules4 { get; set; }
        public string Rules5 { get; set; }
        public string Rules6 { get; set; }
        public string Rules7 { get; set; }
        public string Rules8 { get; set; }
        public string Rules9 { get; set; }
        public string Rules10 { get; set; }
        public string Rules11{ get; set; }
        public string Rules12 { get; set; }
        public string Rules13 { get; set; }
        public string Rules14 { get; set; }
        public string Rules15 { get; set; }
        public string Rules16 { get; set; }
        public string Rules17 { get; set; }
        public string Rules18 { get; set; }
        public string Rules19 { get; set; }
        public string Rules20 { get; set; }
        public string Rules21 { get; set; }
        public string Rules22 { get; set; }

        public object FeedbackSuccessMsg { get; set; }
        public string FeedbackSuccessMsg1 { get; set; }
        public object FeedbackFailMsg { get; set; }
        public string FeedbackFailMsg1 { get; set; }
        public object FeedbackTitle { get; set; }

        public object SendFeedbackMsg { get; set; }
        public object FieldMessage { get; set; }
        public object FieldValMessage { get; set; }
        public string FieldName { get; set; }
        public string FieldValName { get; set; }
        public string FieldEmail { get; set; }
        public string PageTitle { get; set; }
        public string FieldRequired { get; set; }
        public string FieldValEmail { get; set; }
        public string BtnClear { get; set; }
    }
}